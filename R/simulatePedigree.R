...
simulate_IBD_pop_Pedigree <- function(chrompos, PLAF, rho, k){
  ...
  f1.1 <- makecrossover(smpl1 = p1, smpl2 = p2, recombo.block = recombo.block1)
  ...
  f1.2 <- makecrossover(smpl1 = p1, smpl2 = p2, recombo.block = recombo.block2)
  ...
  p1@haplobit <- character() # to match empty above
  p2@haplobit <- character()
  ...
  for(i in 1:length(f1matings)){
    ...
    genx <- makecrossover(smpl1 = f1.1, smpl2 = f1matings[[i]], recombo.block = mate.recombo)
    f1.1 <- genx[[ 1 ]]
  ...
  for(i in 1:length(f2matings)){
    mate.recombo <- findrecombination(chrompos = CHROMPOS, rho = rho)
    genx <- makecrossover(smpl1 = f1.2, smpl2 = f2matings[[i]], recombo.block = mate.recombo)
    f1.2 <- genx[[ 1 ]]
    # TODO always picking the F1 haplotype -- is this fair?
