...
sim_biallelic <- function(haplotypematrix = matrix(1, 2, 2),
  ...
  # assert nrow haplotypematrix is same length as PLAF
  ...
  # get params
  COI <- ncol(haplotypematrix)
  L <- nrow(haplotypematrix)
  # generate strain proportions
  w <- rdirichlet(rep(alpha, COI))

  # generate biallelic table from PLAF
  m <- haplotypematrix
  for(i in 1:nrow(m)){
    hapuni <- unique( m[i, ] )

    if (length(hapuni[! hapuni %in% c(0,1)]) > 0) {
      key <- sample(x = c(0,1), size = length(hapuni[! hapuni %in% c(0,1)])) # multiallelic to biallelic w/ 50-50 for now
      names(key) <- hapuni[! hapuni %in% c(0,1)]
    }
    for(j in 1:length(key)){
      m[i,][ m[i,] == names(key[j]) ] <- key[j]
    }
  }
