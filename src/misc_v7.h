...
//------------------------------------------------
// slice  vector based on a range
// https://www.techiedelight.com/get-slice-sub-vector-from-vector-cpp/
#ifdef RCPP_ACTIVE
template <typename T>
std::vector<T> slice(std::vector<T> const &t, int s, int e) {
  auto start = t.begin() + s;
  auto end = t.begin() + e + 1; // +1 because 0 based

  std::vector<T> vec(start, end);
  return vec;
