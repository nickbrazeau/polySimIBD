...
# Overview
While the discrete-loci, discrete-time spatial structured Wright Fisher model has its own output format, at times it may be beneficial to work with "traditional" genomic data formats. In this tutorial, we will use the model to simulate a infections - with multiple parasites, or with a COI > 1, in ten hosts. We will then will simulate "reads" from a subset of respective host "samples". Reads will produce allele depths for our variants (i.e. in VCF terminology, the AD and DP fields). For the purpose of this tutorial, we will force all sites to be biallelic.  

# Simulating Genomic "Reads" Data and VCFs 
'polySimiIBD' allows the user to interact with individual parasites. As a result, this requires some thought when it comes to combining data within-hosts. Below, we simulate data (as before) for a population of parasites among ten hosts. We then create the ARG for those parasites that are within Host 1 and Host 2. Finally, we collapse the parasite haplotypes from host 1 and 2 into "sequencing read counts", respectively.    
...
# Tree Conversions
Sepearately, `polySimIBD` provides the functionality to convert a \class{bvtree} from the \class{arggraph} into a [Newick tree]() 
