...
#' @noRd
#' Internal use
#' This function takes in two simhaplos (parents) and the recombination block
#' and makes haplobits and haplogt with the simulated crossover from
#' the recombination block -- returns the two sister chromatids
makecrossover <- function(p1, p2, recombo.block){
  out1 <- out2 <- new("simhaplo")

  if( length(unique(recombo.block)) != 1){
    # crossover sister chromatid 1
    out1@haplogt <- p1@haplogt
    out1@haplogt[recombo.block == "I"] <- p2@haplogt[recombo.block == "I"]
    out1@haplobit <- p1@haplobit
    out1@haplobit[recombo.block == "I"] <- p2@haplobit[recombo.block == "I"]

    # crossover sister chromatid 2
    out2@haplogt <- p2@haplogt
    out2@haplogt[recombo.block == "I"] <- p1@haplogt[recombo.block == "I"]
    out2@haplobit <- p2@haplobit
    out2@haplobit[recombo.block == "I"] <- p1@haplobit[recombo.block == "I"]
