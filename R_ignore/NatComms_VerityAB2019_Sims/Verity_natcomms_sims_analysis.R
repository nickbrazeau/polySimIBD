...
tlim <- 10
genome_length <- 23e6
mut_rate <- 2.45e-10 * 23e6/length(pos) 
hosts <- 1:10 
...
saveRDS(paramsdf, "~/Desktop/temp_natcomm/polysim25_plaf_from_wsaf.RDS")
...
plotObj <- plotdf %>% 
  ...
  plot_theme +
  ggtitle("Using Rbeta Calculations as the PLAF (25 reps)")

jpeg("~/Desktop/temp_polysimibd_rbeta.jpg", width = 11, height = 8, units = "in", res = 300)
plot(plotObj)
graphics.off()
